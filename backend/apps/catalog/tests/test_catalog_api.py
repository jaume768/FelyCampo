...
def test_los_borradores_no_se_publican(api, catalog):
    # is_published es un campo derivado (Product.save()) desde Fase 2 — se cambia el
    # estado real, no el booleano directamente, que ahora se sobrescribiría solo.
    catalog["product"].status = ProductStatus.DRAFT
...
def test_el_listado_expone_y_filtra_por_line(api, catalog):
    # `line` por defecto en el modelo es PRET_A_PORTER — ver Product.line, catalog/models.py.
    item = api.get("/api/v1/catalog/products/").json()["results"][0]
    assert item["line"] == "pret_a_porter"

    assert api.get("/api/v1/catalog/products/?line=pret_a_porter").json()["count"] == 1
    assert api.get("/api/v1/catalog/products/?line=atelier").json()["count"] == 0
