...
class Order(UUIDTimeStampedModel):
    ...
    # --- Seguimiento del envío (D4) — solo panel admin, nunca en la API pública: de la
    # logística se encarga una empresa externa y al cliente se le remite al correo/teléfono
    # de la tienda (ver OrderSerializer, "sin estado de envío"). Texto libre: no hay
    # integración con ningún transportista concreto todavía, solo constancia manual.
    tracking_carrier = models.CharField(_("transportista"), max_length=100, blank=True)
    tracking_code = models.CharField(_("código de seguimiento"), max_length=100, blank=True)
    tracking_url = models.URLField(_("enlace de seguimiento"), max_length=500, blank=True)
    ...
    # Regla de "retrasado" del panel admin (Fase 4) — antes vivía en el frontend mock
    # (`esRetrasado`, comparando contra `Date.now()` en cada render); se trae aquí como
    # única fuente de verdad para que sea lo mismo mostrarla en una fila que filtrar u
    # ordenar por ella desde `AdminOrderFilter` (`?is_delayed=true`).
    DELAYED_THRESHOLD_DAYS = 14
    ...
    def is_delayed(self) -> bool:
        ...
        return timezone.now() >= self.created_at + timezone.timedelta(
            days=self.DELAYED_THRESHOLD_DAYS
        )
...
class OrderStatusChange(UUIDTimeStampedModel):
    """
    Auditoría (Fase 4 del panel admin): un cambio real de `Order.status`. Lo crea
    `apps.orders.services.change_order_status` — nunca se escribe a mano, ni siquiera
    desde el propio admin de Django, para que quede constancia de quién y cuándo.
    """

    order = models.ForeignKey(
        Order, verbose_name=_("pedido"), on_delete=models.CASCADE, related_name="status_changes"
    )
    from_status = models.CharField(_("estado anterior"), max_length=24, choices=OrderStatus.choices)
    to_status = models.CharField(_("estado nuevo"), max_length=24, choices=OrderStatus.choices)
    ...
    note = models.CharField(_("motivo"), max_length=255, blank=True)

    class Meta:
        verbose_name = _("cambio de estado de pedido")
        verbose_name_plural = _("cambios de estado de pedido")
        ordering = ["-created_at"]
...
class OrderNote(UUIDTimeStampedModel):
    """
    Nota interna en formato hilo (D6) — sustituye/complementa a `Order.staff_note`
    (campo único, sin autor ni fecha por entrada). `staff_note` se deja intacto porque lo
    sigue usando el flujo de devoluciones (`Return.staff_note` es un campo aparte, pero
    `Order.staff_note` es el que ya escribía el panel antes de esta fase) — no se migra su
    contenido aquí automáticamente, conviven los dos.
    """

    order = models.ForeignKey(
        Order, verbose_name=_("pedido"), on_delete=models.CASCADE, related_name="notes"
    )
    ...
    body = models.TextField(_("nota"))

    class Meta:
        verbose_name = _("nota de pedido")
        verbose_name_plural = _("notas de pedido")
        ordering = ["-created_at"]

    def __str__(self) -> str:
        return f"Nota en {self.order.reference}"
