...
def test_anadir_al_carrito_un_producto_con_foto_no_revienta(api, variant):
    # Regresión: `ProductImage.image` (ImageField) se sustituyó por `ProductImage.asset`
    # (FK a `media.MediaAsset`) en la Fase 2 del panel admin — `CartItemSerializer.get_image`
    # se quedó leyendo el campo viejo y reventaba con AttributeError en cuanto un producto
    # del carrito tenía de verdad una imagen (antes nunca pasaba: seed_demo no sube fotos).
    asset = MediaAsset.objects.create(
        kind=MediaKind.IMAGE,
        file=SimpleUploadedFile("foto.jpg", b"contenido-de-prueba", content_type="image/jpeg"),
    )
    ProductImage.objects.create(product=variant.colorway.product, asset=asset)

    response = api.post("/api/v1/cart/", {"variant": str(variant.id), "quantity": 1}, format="json")

    assert response.status_code == 200
    image_url = response.json()["items"][0]["image"]
    # El nombre de archivo se normaliza al subir (hash, no "foto.jpg") — lo que importa es
    # que resuelva una URL real de la biblioteca en vez de reventar con AttributeError.
    assert image_url is not None
    assert "/media/library/" in image_url
