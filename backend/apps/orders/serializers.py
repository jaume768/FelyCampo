...
class CartItemSerializer(serializers.ModelSerializer):
    ...
    def get_image(self, obj) -> str | None:
        # `ProductImage.image` (ImageField) se sustituyó por `ProductImage.asset` (FK a
        # `media.MediaAsset`) en la Fase 2 del panel admin — el archivo real vive en
        # `asset.file`, no en un campo `image` propio (ver apps/catalog/serializers.py,
        # `ProductImageSerializer`, que ya usa esta misma ruta).
        image = obj.variant.colorway.images.first() or obj.variant.colorway.product.images.first()
        if image is None:
            return None
        # No `request.build_absolute_uri()` — ver apps/core/media_urls.py: la petición
        # que arma el carrito puede venir del cliente directo, pero el mismo serializer
        # se reutiliza en cualquier contexto, así que se trata igual que el catálogo.
