...
REST_FRAMEWORK = {
    ...
    "DEFAULT_THROTTLE_RATES": {
        ...
        # Panel admin: personal identificado (IsStaff), no tráfico anónimo — cupo propio
        # y más generoso que "user", para no chocar con el límite del sitio público.
        "admin": env("THROTTLE_ADMIN", default="300/min"),
    },
}
...
SPECTACULAR_SETTINGS = {
    ...
    # Dos campos distintos se llaman "kind" (Product.kind y media.MediaAsset.kind, con
    # choices distintas). Antes de que existiera MediaKind, Product.kind se resolvía solo
    # como "KindEnum" (único choice set con ese nombre de campo, sin ambigüedad posible);
    # al añadir un segundo choice set con el mismo nombre de campo, drf-spectacular deja
    # de poder resolver NINGUNO de los dos automáticamente — hace falta nombrar ambos a
    # mano, no solo el nuevo, o "KindEnum" pasaría a un hash generado también.
    "ENUM_NAME_OVERRIDES": {
        "KindEnum": "apps.catalog.models.ProductKind",
        "MediaKindEnum": "apps.media.models.MediaKind",
        "LocationKindEnum": "apps.stock.models.LocationKind",
        # Mismo choque que "kind" arriba, ahora con "status": ProductStatus ya lo usaba en
        # solitario (resolvía solo a "StatusEnum"); al añadir Order.status/Return.status
        # (Fase 4, panel de pedidos) hacen falta los tres nombrados a mano.
        "StatusEnum": "apps.catalog.models.ProductStatus",
        "OrderStatusEnum": "apps.orders.models.OrderStatus",
        "ReturnStatusEnum": "apps.orders.models.ReturnStatus",
    },
}
...
# URL pública de ESTA API — la que ve un navegador real, no el nombre de servicio Docker
# (`backend`) que usa el frontend al pedir el catálogo desde su Server Component (ver
# apps/core/media_urls.py). Mismo valor que NEXT_PUBLIC_API_URL del frontend en dev.
BACKEND_PUBLIC_URL = env("BACKEND_PUBLIC_URL", default="http://localhost:8001")
